...
**Current build status:** ✅ `forge build` green · ✅ `forge test` **135/135** passing (105 unit/invariant + 30 fork) · ✅ `tsc` zero errors · ✅ **rc5 contracts + rc6 Relayer live on Sepolia + Hyperlane full pipeline validated end-to-end (both LP-execute path AND zero-delta short-circuit path)** · ✅ CI green on `main` · ✅ All hardening (R-1..R-13) live on chain · ✅ rc5 Relayer math + CurrencySettler proven (tx `0x6e3bb567…7381`) · ✅ rc5 hook.handle() proven (tx `0x49cb5e21…6e54`) · ✅ **rc6 zero-delta short-circuit proven (tx `0xeee761e9…5f13`, 154k gas, no V4 modify)** · ✅ Monitoring stubs prepared · 🟡 Phase 5.B agent soak: deterministically verified; live tsx soak deferred on system load.

**rc5/rc6 testnet addresses** (redeployed 2026-05-18 with R-1..R-13 hardening + Relayer TickMath fix; Relayer bumped to rc6 same day for zero-delta short-circuit):
- Base Sepolia: Treasury `0x00288400…0392` · Hook `0xA059C854…c540` · Vault `0x062b9E54…f37b` · Factory `0xC3e117CD…C2A7`
- ETH Sepolia:  Hook `0xc3233eb9…8540` · Relayer `0x5D7BA93B…b727` (rc6; supersedes rc5 `0x72e2538a…0155`)
...
- **Full cross-chain pipeline validated end-to-end on rc5 contracts 2026-05-18**: bootstrap LP seeded on both pools (L=2e9 each), Hyperlane delivered 2 messages live:
  ...
  - Message 2 (Base→ETH LP-callback depth notification, `0x2f3be7ba…216b`) was stuck because it carried `deltaToken0=deltaToken1=0` and pre-rc6 Relayer let it reach V4 modifyLiquidity which reverts on zero liquidity. **Fixed at rc6**: `Relayer._executeRebalance` now short-circuits on zero deltas, emits `RebalanceSkippedZeroDelta`, returns cleanly. (Existing pinned `0x2f3be7ba…216b` cannot deliver because its message body targets the old Relayer address — abandoned.)
- **rc6 Relayer redeployed + rewired on ETH Sepolia 2026-05-18**: new Relayer at `0x5D7BA93B…b727` (replaces v5/rc5 `0x72e2538a…0155`). Re-registered pool, updated Base hook's sister to new address, swapped Vault chain registry's cctpRecipient (via removeChain + addChain), migrated 10 USDC + 0.001 WETH from old → new via `rescueToken` + transfer. Live end-to-end validation of rc6 short-circuit: zero-delta dispatch tx `0x64a22b1c…4762` (Base) → messageId `0xc73e52c6…8de0` → Hyperlane delivered automatically in ~30s to ETH tx `0xeee761e9…5f13` → fired `MessageReceived` + `RebalanceSkippedZeroDelta` + zero `RebalanceExecuted` ✓. Gas used: 154k (~60% less than full-execute path).
...
- [x] **Pre-audit hardening pass landed 2026-05-18** — R-1 (bound `updateCrossChainAssets` delta) + R-5 (timelock on `setTreasury` / `setMailbox` / `setSafe`) + withdrawEth regression test + audit doc updates. Re-tagged as `v1.0.0-rc2`.
- [x] **Second hardening pass 2026-05-18** — R-3 (harvest-staleness gate on cross-chain report), R-10 (CCTP recipient required when `cctpDomain != 0` in `addChain`), R-12 (cache `chainlinkFeed.decimals()` in `MirrorHook` constructor). Re-tagged as `v1.0.0-rc3`.
- [x] **Third hardening pass 2026-05-18** — R-7 (guardian role for fast-response pause on Vault + Hook; unpause stays owner-only), R-11 (Pyth/Chainlink cross-oracle deviation check at 5% default; `_getOraclePrice` refactored into `_tryPythPrice` + `_tryChainlinkPrice` + cross-check), R-13 (cap on sister-reported `currentDepth` at `maxSisterDepthMultiple × prior` with `SisterDepthCapped` event). Plus `audits/OPERATIONS.md` runbook covering R-4 (agent key rotation), R-6 (multisig signing policy), R-9 (authorized-sender admin sequence). Re-tagged as `v1.0.0-rc4`.
- [x] **Fourth hardening pass 2026-05-18 — Relayer execution math (rc5)** — closed the long-standing pre-mainnet blocker. `Relayer._liquidityFromDeltas` had been a placeholder that returned `int256(d0)` (sign-correct, magnitude wrong); `_settleDeltas` had the V4 sign convention INVERTED (treated "caller owes" as "pool owes") AND used bare `currency.transfer` instead of `sync + transfer + settle`. Both latent because no test prior to rc5 exercised `_executeRebalance` end-to-end. Fixed via TickMath + LiquidityAmounts + StateLibrary.getSlot0 for the math; CurrencySettler (OZ uniswap-hooks helper) for the settle path. New fork test `test_relayerExecutesRebalanceWithProperLiquidityMath` is the first end-to-end coverage of the path. Re-tagged as `v1.0.0-rc5`.
- [x] **Invariant runner-gap closures 2026-05-18** — extended `VaultHandler` with new actions (`addChainAndRebalance`, `rebalanceAllocations`, `removeZeroAllocChain`, `proposeTreasury`/`executeTreasury`/`cancelPendingTreasury`, `ownerPause`/`guardianPause`) and added three new invariants: `invariant_allocationSumIsZeroOrFull` (closes I-5 runner-gap — sum across enabled chains is either 0 or exactly 10_000), `invariant_pendingTreasuryConsistency` (pendingTreasury and pendingTreasuryEffectiveAt are written/cleared together), `invariant_guardianStillConfigured` (R-7 fixture-drift sanity check). `forge test` **134/134** green across 256-run fuzz × 14 selectors. Slither stable.
- [x] **rc5 testnet redeploy 2026-05-18** — abandoned the v5 deploy (rc1-era contracts without R-1..R-13 protections and with broken Relayer math). Deployed full rc5 stack: new Treasury + Hook + Vault + Factory on Base Sepolia, new Hook + Relayer on ETH Sepolia. Hook salts re-mined (Base 46146, ETH 8680) since rc5 bytecode differs from v5. Verified on-chain: R-1 maxDeltaBps=2500, R-3 staleness=3600s, R-5 timelock=86400s, R-11 oracleTol=500bps, R-13 sister-cap=10x, allocation Base 6000 BPS / ETH 4000 BPS, sister allowlist symmetric, V4 pools initialized on both chains at tick 199799. Verification on Etherscan partial (one hook needs manual retry — cosmetic; contract works). Hyperlane dispatch verified end-to-end source-side (`0x8a8841d1…000f`).
- [x] **Monitoring stubs 2026-05-18 (Phase 8 prep)** — new `monitoring/` directory:
  - `tenderly/alerts.yml` — 12 alert rules matching OPERATIONS.md §7 (large delta event, delta-rejected burst, treasury/mailbox/safe proposals, agent EOA outside call set, DispatchFailed, OracleDeviationTooLarge, SisterDepthCapped, vault/hook pause, hook + agent low-ETH).
  - `tenderly/abi.json` — event + revert signatures referenced by alerts.yml, sourced from rc5 artifacts.
  - `dune/01..05_*.sql` — Total Mirrored TVL, weekly extra APY, rebalance-event feed, treasury fee-share flow, pause-state changes (incident-response audit trail).
  - `telegram/heartbeat-spec.md` — spec for the agent → Telegram heartbeat job (loud-on-action + missing-heartbeat-is-the-alert pattern).
  All stubs use `{{ ENV_VAR }}` placeholders so no live addresses are committed. Wire to real Tenderly + Dune workspaces at Phase 9.
