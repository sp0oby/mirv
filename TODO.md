...
**Current build status:** ✅ `forge build` green · ✅ `forge test` **131/131** passing (101 unit/invariant + 30 fork) · ✅ `tsc` zero errors · ✅ v5 testnet live on Base Sepolia + ETH Sepolia, all 6 contracts verified · ✅ CI green on `main` · ✅ Phase A (canonical pairId dispatch) + Phase D (CCTP deposit) validated end-to-end on v5 · ✅ Pre-audit hardening R-1/R-2/R-3/R-5/R-7/R-10/R-11/R-12/R-13 landed, R-4/R-6/R-9 in OPERATIONS.md, Relayer TickMath + V4-settle fix landed, tagged `v1.0.0-rc5` · 🟡 Phase 5.B agent soak: 6-bug audit + deterministic cast verification landed; confirming live-agent run deferred until system is free.
...
- [x] **Fourth hardening pass 2026-05-18 — Relayer execution math (rc5)** — closed the long-standing pre-mainnet blocker. `Relayer._liquidityFromDeltas` had been a placeholder that returned `int256(d0)` (sign-correct, magnitude wrong); `_settleDeltas` had the V4 sign convention INVERTED (treated "caller owes" as "pool owes") AND used bare `currency.transfer` instead of `sync + transfer + settle`. Both latent because no test prior to rc5 exercised `_executeRebalance` end-to-end. Fixed via TickMath + LiquidityAmounts + StateLibrary.getSlot0 for the math; CurrencySettler (OZ uniswap-hooks helper) for the settle path. New fork test `test_relayerExecutesRebalanceWithProperLiquidityMath` is the first end-to-end coverage of the path. `forge test` **131/131** green (was 130; +1 new fork test). Slither: same 7 high/medium in-scope. Re-tagged as **`v1.0.0-rc5`** — current audit candidate.
