...
**Current build status:** ✅ `forge build` green · ✅ `forge test` **119/119** passing (97 unit/invariant + 22 fork) · ✅ `tsc` zero errors · ✅ v5 testnet live on Base Sepolia + ETH Sepolia, all 6 contracts verified · ✅ CI green on `main` · ✅ Phase A (canonical pairId dispatch) + Phase D (CCTP deposit) validated end-to-end on v5 · ✅ Pre-audit hardening R-1/R-2/R-3/R-5/R-10/R-12 landed, tagged `v1.0.0-rc3` · 🟡 Phase 5.B agent soak: 6-bug audit + deterministic cast verification landed; confirming live-agent run deferred until system is free.
...
- [x] **Second hardening pass 2026-05-18** — R-3 (harvest-staleness gate on cross-chain report), R-10 (CCTP recipient required when `cctpDomain != 0` in `addChain`), R-12 (cache `chainlinkFeed.decimals()` in `MirrorHook` constructor). `forge test` **119/119** green (was 111; +8 new tests). Slither: same 7 high/medium in-scope as rc2, 0 new findings. Re-tagged as **`v1.0.0-rc3`** — current audit candidate.
