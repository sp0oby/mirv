...
function DepositForm() {
  ...
  const { data: allowance, refetch: refetchAllowance } = useReadContract({
    address: USDC_BASE_SEPOLIA, abi: erc20Abi, functionName: "allowance",
  ...
  const { data: previewShares } = useReadContract({
    address: MIRROR_VAULT_BASE, abi: VAULT_DEPOSIT_ABI, functionName: "previewDeposit",
    args: [amountUnits], query: { enabled: amountUnits > 0n && !wrongChain },
  });
  ...
  function onApprove() {
    doApprove({
      address: USDC_BASE_SEPOLIA, abi: erc20Abi, functionName: "approve",
  ...
  function onDeposit() {
    if (!address) return;
    doDeposit({
      address: MIRROR_VAULT_BASE, abi: VAULT_DEPOSIT_ABI, functionName: "deposit",
  ...
  if (!isConnected) {
    primaryLabel = ""; primaryOnClick = undefined; primaryDisabled = true;
  } else if (wrongChain) {
    ...
    <div className="pt-4">
      <header className="mb-10">
        ...
        <p className="text-[16px] text-ink-soft max-w-[60ch]">
          deposit usdc on base sepolia. shares mint roughly 1:1 on first
          deposit; share price after that reflects total assets / total
      ...
      <section className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-10">
        <div className="frame-outer p-7 bg-paper-warm" style={{ transform: "rotate(-0.4deg)" }}>
          <div className="flex items-center justify-between mb-3">
            <label className="font-maru text-[14px] text-ink font-semibold">amount</label>
          ...
          {amountUnits > 0n && previewShares !== undefined && (
            <p className="text-[13px] text-ink-soft mb-5">
              you would receive ~<span className="font-mono text-ink"> {(Number(previewShares) / 1e6).toFixed(4)} </span>mirvUSDC shares.
            </p>
          )}
        ...
        <aside className="space-y-3">
          <div className="frame-outer p-4" style={{ transform: "rotate(0.5deg)" }}>
            <p className="font-maru text-[11px] uppercase tracking-wider text-ink-faint mb-1">vault</p>
            <p className="font-mono text-[11px] text-ink break-all">{MIRROR_VAULT_BASE}</p>
          </div>
          <div className="frame-outer p-4" style={{ transform: "rotate(-0.5deg)" }}>
            <p className="font-maru text-[11px] uppercase tracking-wider text-ink-faint mb-1">chain</p>
            <p className="text-[14px] text-ink">base sepolia</p>
          </div>
          <div className="frame-outer p-4" style={{ transform: "rotate(0.3deg)" }}>
            <p className="font-maru text-[11px] uppercase tracking-wider text-ink-faint mb-1">allocations</p>
            <p className="text-[13px] text-ink-soft">60% base · 40% eth sepolia</p>
            <p className="text-[11px] text-ink-faint mt-1">eth share bridges via cctp.</p>
          </div>
        </aside>
      </section>

      <p className="text-[12px] text-ink-faint mt-10">testnet — tokens have no value. dyor before mainnet.</p>
    </div>
  );
}
