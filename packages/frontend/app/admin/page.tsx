...
// Admin v0 — read-only inspector of owner-controlled state on Vault + Hook.
// Write actions (pause, setMaxCrossChainAssetsDeltaBps, proposeTreasury,
// setGuardian, setAllocations) are real but staying behind a "show
// dangerous actions" toggle until we wire the confirmation modals + a
// "this is testnet" banner. mainnet admin will be the multisig anyway.
...
export default function AdminPage() {
  ...
  const { data: pendingEffectiveAt }    = useReadContract({ address: MIRROR_VAULT_BASE, abi: VAULT_VIEW_ABI, functionName: "pendingTreasuryEffectiveAt", query: { enabled } });
    ...
    <div className="pt-4">
      <header className="mb-10">
        ...
        <p className="text-[16px] text-ink-soft max-w-[60ch]">
          read-only inspector of the protocol's owner-controlled state.
          dangerous actions (pause, propose new treasury, set bounds) go
          here once we wire confirmation modals — for now this is a
          transparency view so anyone can verify the live parameters.
        </p>
      </header>
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-[14px]">
            ...
            <Row label="pending treasury"      value={
              pendingTreasury && pendingTreasury !== "0x0000000000000000000000000000000000000000"
                ? `${shortAddr(pendingTreasury)} (effective at ${pendingEffectiveAt ? new Date(Number(pendingEffectiveAt) * 1000).toLocaleString() : "?"})`
                : "(none pending)"
            } />
      ...
      <p className="text-[12px] text-ink-faint">
        write actions land in v1 with explicit confirmation modals + a
        "this changes live state" banner. testnet, no real money.
      </p>
    </div>
  );
}
