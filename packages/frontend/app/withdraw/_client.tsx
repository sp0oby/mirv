...
function WithdrawContent() {
  ...
  const { data: previewAssets } = useReadContract({
    ...
    args: amountUnits > 0n ? [amountUnits] : undefined,
    query: { enabled: enabled && amountUnits > 0n },
  });
    ...
    <div className="pt-4">
      <header className="mb-10">
        ...
        <p className="text-[16px] text-ink-soft max-w-[60ch]">
          burn ur mirvUSDC shares for the underlying. sync if base has
          enough usdc; otherwise the shares queue and the agent unwinds
          a sister LP first.
        </p>
      </header>
      ...
      {!isConnected && (
        <section className="frame-outer p-8 bg-paper-warm text-center" style={{ transform: "rotate(0.4deg)" }}>
          <p className="text-[16px] text-ink mb-5">connect ur wallet to withdraw.</p>
      ...
      {enabled && (
        <section className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-10">
          <div className="frame-outer p-7 bg-paper-warm" style={{ transform: "rotate(-0.3deg)" }}>
            ...
            <label className="font-maru text-[14px] text-ink font-semibold block mb-3">shares to burn</label>
            <div className="flex items-center justify-between mb-2 text-[12px]">
              ...
              {shareBalance !== undefined && shareBalance > 0n && (
                <button onClick={() => setAmount(formatUnits(shareBalance, 6))} className="text-pink-hot underline">
                  use max
                </button>
              )}
            </div>
            ...
            {amountUnits > 0n && (
              <p className="text-[14px] text-ink-soft mb-5">
                you would receive ~<span className="font-mono text-ink"> ${wouldGetNum.toFixed(2)} </span>usdc.
              </p>
            )}
          ...
          <aside className="space-y-3">
            <div className="frame-outer p-4">
              <p className="font-maru text-[11px] uppercase tracking-wider text-ink-faint mb-1">local liquidity</p>
            ...
            <div className="frame-outer p-4 bg-paper-warm">
              ...
              <ul className="text-[12px] text-ink-soft space-y-1 leading-snug">
                <li>— shares park in vault custody, not yet burned</li>
                <li>— agent unwinds sister LP, bridges back via cctp</li>
                <li>— agent calls fulfillWithdraw, paid at current price</li>
                <li>— cancellable after 24h if agent stalls</li>
              </ul>
            </div>
          </aside>
        </section>
