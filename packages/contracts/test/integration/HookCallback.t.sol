...
contract HookCallbackTest is Test {
    ...
    function setUp() public {
        ...
        try vm.rpcUrl("base") returns (string memory u) { rpcUrl = u; } catch { vm.skip(true); return; }
        if (bytes(rpcUrl).length == 0 || _equals(rpcUrl, "${ALCHEMY_BASE_URL}")) { vm.skip(true); return; }
        ...
        vm.mockCall(
            HYPERLANE_MAILBOX,
            abi.encodeWithSignature("quoteDispatch(uint32,bytes32,bytes)"),
